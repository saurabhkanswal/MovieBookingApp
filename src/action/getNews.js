...
export const getTopHeadline = () => async dispatch => {
  const base_url = `https://newsapi.org/v2/top-headlines?country=in&category=entertainment&apiKey=${API_KEY}`;
  console.log('enterd');
  try {
...
export const getTopThreeTrendingNews = () => async dispatch => {
  const base_url = `https://newsapi.org/v2/top-headlines?country=us&category=entertainment&pageSize=3&apiKey=${API_KEY}`;
  console.log('enterd');
  try {
    const {data} = await Axios.get(base_url);
    console.log(data);
